	...
	class LspApp : IDEApp {
		...
		private void LoadWorkspaceUserDataCustom() {
			...
			if (sd.Load(path) case .Err(let err)) {
				Log.Error($"Failed to load workspace user data at '{path}' ({err})");
				return;
			}
		...
		public void SaveWorkspaceUserDataCustom() {
			...
			StructuredData sd = scope .();
			if (sd.Load(path) case .Err(let err)) {
				Log.Error($"Failed to save workspace user data at '{path}' ({err})");
				if (err != .FileError || File.Exists(path)) 
					return;
			}
			if (sd.IsEmpty) {
				...
				(sd.Get("LastConfig") as String)?.Set(mConfigName);
				(sd.Get("LastPlatform") as String)?.Set(mPlatformName);
			}

			String data = sd.ToTOML(.. scope .());
			if (!SafeWriteTextFile(path, data, false)) 
				Log.Error($"Failed to write workspace user data at '{path}'");
		}
