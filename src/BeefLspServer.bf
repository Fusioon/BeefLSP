...
namespace BeefLsp {
	...
	class BeefLspServer : LspServer {
		...
		private Result<Json, Error> OnChangePlatform(Json args)
		{
			StringView platform = args["platform"].AsString;

			if (app.mPlatformName != platform) {
				let platformList = app.mWorkspace.GetPlatformList(.. scope .());

				if (platformList.ContainsAlt(platform)) {
					app.mPlatformName.Set(platform);
					app.mWorkspace.FixOptions();
					app.SaveWorkspaceUserDataCustom();
		...
		private Result<Json, Error> OnPlatforms() {
			Json json = .Array();

			let platformList = app.mWorkspace.GetPlatformList(.. scope .());
			for (let platform in  platformList) {
				json.Add(.String(platform));
			}
		...
		protected override void OnMessage(Json json) {
			...
			case "beef/changePlatform":          	  HandleRequest(json, OnChangePlatform(args));
			...
			case "beef/platforms":               	  HandleRequest(json, OnPlatforms());
